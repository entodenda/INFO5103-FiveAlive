...
import { StyleSheet, Image, Platform, View, FlatList, TouchableOpacity, Text } from "react-native";
...
const RecipesScreen: React.FC = () => {
  ...
  const renderRecipeItem = ({ item }: { item: Recipe }) => (
    <TouchableOpacity onPress={() => handleRecipePress(item)} style={styles.recipeItem}>
      {item.image && (
        <Image
        ...
        <View style={styles.filtersContainer}>
          ...
          {(Object.values(DietTag) as DietTag[]).map(tag => (
            <TouchableOpacity key={tag} onPress={() => toggleTag(tag)}>
              <Text style={selectedTags.includes(tag) ? styles.selectedTag : styles.tag}>
                {DietTag[tag]}
              </Text>
            </TouchableOpacity>
          ))}
