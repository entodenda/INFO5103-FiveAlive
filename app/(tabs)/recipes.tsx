...
const RecipesScreen: React.FC = () => {
  ...
  useEffect(() => {
    const updateRecipes = async () => {
      if (ingredients.length !== 0) {
        try {
          ...
          let res: Recipe[] | null = await FindMatchingRecipe([], ids); // TODO: Diet tags go here
          if (!res || res.length === 0) {
            res = recipes;
          }
    ...
    setTotalPages(Math.ceil(filteredAndSortedRecipes.length / recipesPerPage));
    updateRecipes();
  }, [ingredients, selectedSortLabel]);
